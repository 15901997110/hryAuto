...
public enum ReplaceRegexEnum {
     ...
     */
    DATETIME("(?i)<datetime\\(-?\\d*\\)>", "<datetime(numHour)>"),

    /**
     * <time(num)> 生成一个时间,格式"HH:mm:ss"
     * 参数:num-非必填,指定时间按分钟的偏移量
     */
    TIME("(?i)<time\\(-?\\d*\\)>", "<time(numMinute)>"),
     ...
     */
    RANDOM_I("(?i)<randomI\\(\\d*,?\\d*\\)>", "<randomI(min,max)>"),
     ...
     */
    SOURCE("<\\$\\d+\\([^<>]*\\)>", "<$index(xxx)>"),//匹配<$10(helloWorld)>

    /**
     * <ref(index)> 与<$index(xxx)>配合使用,引用xxx内容
     */
    REF("(?i)<ref\\(\\d+\\)>", "<ref(index)>"),//匹配<ref(10)>
    ...
    ReplaceRegexEnum(String pattern, String example) {
